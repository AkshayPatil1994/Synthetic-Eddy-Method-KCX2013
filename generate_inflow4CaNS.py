...
covar_org = np.loadtxt('covar_re350.dat',skiprows=1)
...
for i in range(2,np.shape(mean_org)[1]):
    mean[:,i] = np.interp(myz,mean_org[:,1],mean_org[:,i])

for j in range(2,np.shape(covar_org)[1]):
    covar[:,j] = np.interp(myz,mean_org[:,1],covar_org[:,j])
# Setup the z axis
mean[:,1] = myz
covar[:,1] = myz
# Plotting check
if(plotData):
    ...
    plt.plot(mean_org[:,2],mean_org[:,1],'kd',label='reference',markerfacecolor='None')
    plt.plot(mean[:,2],mean[:,1],'r+',label='coarse',markerfacecolor='None')
    plt.legend()
    plt.figure(2)
    plt.title('Covariance check')
    for j in range(2,np.shape(covar_org)[1]):
        plt.plot(covar_org[:,j],covar_org[:,1],'kd',label='reference',markerfacecolor='None')
